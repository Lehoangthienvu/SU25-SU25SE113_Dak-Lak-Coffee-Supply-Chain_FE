...
export default function ManagerInboundRequestsPage() {
  ...
  const filtered = useMemo(() => {
    return items.filter((r) => {
      ...
      const text = `${r.farmerName} ${r.batchCode ?? ""} ${r.coffeeTypeName ?? ""}`.toLowerCase();
          ...
          <div className="flex items-center justify-between gap-4 mb-6">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                ...
                <Input 
                  placeholder="Tìm theo nông dân, lô, loại cà phê..." 
                  value={search} 
                  onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} 
                  className="w-72 pl-10 border-blue-200 focus:border-blue-500 focus:ring-blue-200" 
                />
              </div>
                    ...
                    <tbody>
                      {pageSlice.map((r) => (
                        <tr key={r.inboundRequestId} className="border-b border-gray-100 hover:bg-blue-50 transition-colors">
                          ...
                          <td className="px-4 py-3 text-gray-700">{r.coffeeTypeName || "-"}</td>
