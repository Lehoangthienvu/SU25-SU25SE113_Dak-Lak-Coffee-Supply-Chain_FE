...
export default function BusinessStaffListPage() {
  ...
  useEffect(() => {
    async function fetchStaffs() {
      try {
        ...
        toast.error(t('businessStaffs.error.loadList'));
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleDelete = async (id: string) => {
    const confirm = window.confirm(t('businessStaffs.delete.confirm'));
    if (!confirm) return;

    try {
      const res = await softDeleteBusinessStaff(id);
      if (res.status === 200) {
        toast.success(t('businessStaffs.delete.success'));
        setStaffs((prev) => prev.filter((s) => s.staffId !== id));
      } else {
        toast.error(res.message || t('businessStaffs.delete.error'));
      }
    } catch (err) {
      toast.error(t('businessStaffs.delete.deleteError'));
    }
  };
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold bg-gradient-to-r from-orange-600 to-amber-600 bg-clip-text text-transparent">
                {t('businessStaffs.page.title')}
              </h1>
              <p className="text-gray-600 mt-1 text-sm">
                {t('businessStaffs.page.subtitle')}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <div className="bg-white/80 backdrop-blur-sm rounded-lg p-3 shadow-md">
              <Badge variant="secondary" className="bg-gradient-to-r from-orange-100 to-amber-100 text-orange-800 border-orange-200 text-sm px-3 py-1">
                ...
                {t('businessStaffs.page.staffCount', { filtered: filteredStaffs.length, total: staffs.length })}
              </Badge>
            </div>
            <Button
            ...
            >
              ...
              {t('businessStaffs.page.addStaff')}
            </Button>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-3">
            <div className="relative flex-1">
              ...
              <Input
                placeholder={t('businessStaffs.page.searchPlaceholder')}
                value={searchTerm}
            ...
            >
              ...
              {t('businessStaffs.page.filterButton')} {showFilters ? "↑" : "↓"}
            </Button>
            {hasActiveFilters && (
              <Button 
                variant="ghost" 
              ...
              >
                ...
                {t('businessStaffs.page.clearFilters')}
              </Button>
            )}
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-gray-100">
              <div className="space-y-2">
                <label className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                  ...
                  {t('businessStaffs.page.department')}
                </label>
                <Select value={selectedDepartment} onValueChange={setSelectedDepartment}>
                  <SelectTrigger className="border-2 border-gray-200 focus:border-orange-400 focus:ring-orange-200 rounded-lg py-2 text-sm">
                    <SelectValue placeholder={t('businessStaffs.page.allDepartments')} />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="all">{t('businessStaffs.page.allDepartments')}</SelectItem>
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold text-gray-700">
                  {t('businessStaffs.page.statistics')}
                </label>
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg p-3 border border-gray-200">
                    <div className="text-gray-600 text-xs">{t('businessStaffs.page.total')}</div>
                  ...
                  <div className="bg-gradient-to-br from-orange-50 to-amber-100 rounded-lg p-3 border border-orange-200">
                    <div className="text-orange-600 text-xs">{t('businessStaffs.page.filtered')}</div>
            ...
            <div className="flex flex-wrap gap-2 pt-3">
              {searchTerm && (
                <Badge variant="secondary" className="bg-gradient-to-r from-blue-100 to-blue-200 text-blue-800 border-blue-300 px-3 py-1 text-xs">
                  {t('businessStaffs.page.searchFilter', { term: searchTerm })}
                  <button 
              ...
              {selectedDepartment !== "all" && (
                <Badge variant="secondary" className="bg-gradient-to-r from-green-100 to-green-200 text-green-800 border-green-300 px-3 py-1 text-xs">
                  {t('businessStaffs.page.departmentFilter', { department: selectedDepartment })}
                  <button 
      ...
      {loading ? (
        <Card className="p-12 text-center bg-white/70 backdrop-blur-md rounded-xl shadow-lg">
          ...
          <p className="text-gray-600 mt-4 text-sm">{t('businessStaffs.loading.list')}</p>
        </Card>
      ) : (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gradient-to-r from-orange-50 via-amber-50 to-yellow-50 border-b border-orange-100">
                <tr>
                  <th className="px-6 py-4 text-left text-sm font-bold text-gray-800">{t('businessStaffs.table.headers.staffCode')}</th>
                  <th className="px-6 py-4 text-left text-sm font-bold text-gray-800">{t('businessStaffs.table.headers.fullName')}</th>
                  <th className="px-6 py-4 text-left text-sm font-bold text-gray-800">{t('businessStaffs.table.headers.email')}</th>
                  <th className="px-6 py-4 text-left text-sm font-bold text-gray-800">{t('businessStaffs.table.headers.department')}</th>
                  <th className="px-6 py-4 text-left text-sm font-bold text-gray-800">{t('businessStaffs.table.headers.position')}</th>
                  <th className="px-6 py-4 text-center text-sm font-bold text-gray-800">{t('businessStaffs.table.headers.actions')}</th>
                </tr>
              </thead>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex justify-center items-center gap-2">
                        <Button
                          ...
                          title={t('businessStaffs.table.actions.view')}
                        >
                        ...
                        <Button
                          ...
                          title={t('businessStaffs.table.actions.edit')}
                        >
                        ...
                        <Button
                          ...
                          title={t('businessStaffs.table.actions.delete')}
                        >
                    ...
                    <td colSpan={6} className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center">
                        ...
                        <p className="text-gray-600 text-lg font-semibold mb-2">
                          {hasActiveFilters ? t('businessStaffs.empty.noResults') : t('businessStaffs.empty.noStaff')}
                        </p>
                        <p className="text-gray-500 text-sm">
                          {hasActiveFilters ? t('businessStaffs.empty.noResultsDesc') : t('businessStaffs.empty.noStaffDesc')}
                        </p>
                        {hasActiveFilters && (
                          <Button 
                            variant="outline" 
                          ...
                          >
                            ...
                            {t('businessStaffs.page.clearAllFilters')}
                          </Button>
                        )}
                      </div>
                    </td>
