...
import { Search, Eye, TrendingUp, TrendingDown, Activity, Calendar, Package, Warehouse, Loader2, ArrowLeft } from "lucide-react";
...
export default function StaffInventoryLogsPage() {
  ...
  const logsPerPage = 4; // Hiển thị 4 log mỗi trang
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t('inventoryLogs.error.description') + ": " + err.message);
    } finally {
      setIsLoading(false);
    }
  }, []);
        ...
        <div className="bg-gradient-to-r from-green-500 to-emerald-600 rounded-xl p-4 text-white shadow-lg">
             ...
             <div className="hidden md:block">
               <Button 
                 ...
                 className="bg-white/10 border-white/30 text-white hover:bg-white/20 hover:border-white/50 hover:text-white transition-all duration-200"
               >
                 <ArrowLeft className="w-4 h-4 mr-2" />
                 {t('inventoryLogs.backToDashboard')}
               </Button>
             </div>
           </div>
        </div>
            ...
            <div className="bg-green-50 px-3 py-2 border-t border-green-100">
              <div className="flex flex-col sm:flex-row justify-between items-center gap-2">
                <div className="text-xs text-gray-600">
                  {t('inventoryLogs.pagination.showing')} {((currentPage - 1) * logsPerPage) + 1} {t('inventoryLogs.pagination.to')} {Math.min(currentPage * logsPerPage, filteredLogs.length)} {t('inventoryLogs.pagination.of')} {filteredLogs.length} {t('inventoryLogs.pagination.logs')}
                </div>
